...
try:
    ...
    with tab1:
        ...
        df_dash = df_stok[df_stok["DEPARTEMEN"].isin(dept_f) & df_stok["LOKASI"].isin(lokasi_f)]
        
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Total SKU", f"{len(df_dash):,}")
        c2.metric("Total Val Jual", f"Rp {df_dash['VALSELLING'].sum():,.0f}")
        c3.metric("Qty Selisih", f"{df_dash['QTYSELISIH'].sum():,}", delta_color="inverse")
        c4.metric("Val Selisih", f"Rp {df_dash['SELISIHVALSELLING'].sum():,.0f}", delta_color="inverse")
    ...
    with tab2:
        ...
        if search_loc:
            lokasi_list = [l for l in lokasi_list if search_loc.upper() in str(l).upper()]

        # --- LIST LOKASI ---
        for idx, loc in enumerate(lokasi_list):
            df_loc = df_audit[df_audit["LOKASI"] == loc]
            
            # Cek status dinamis dari kolom JENIS_PENGHITUNG
            tersedia = df_loc["JENIS_PENGHITUNG"].unique()
        ...
        if st.session_state.selected_lokasi:
            ...
            # Kita ambil nilai QTYFISIK untuk setiap JENIS_PENGHITUNG
            ...
            # Pastikan kolom P1, P2, P3 ada (meskipun datanya kosong)
            for p in ['P1', 'P2', 'P3']:
                if p not in df_pivot.columns:
                    df_pivot[p] = 0

            # Hitung Selisih (Menggunakan P2 sebagai final jika ada, jika tidak P1)
            df_pivot['FINAL'] = df_pivot['P2'].where(df_pivot['P2'] > 0, df_pivot['P1'])
            df_pivot['SELISIH'] = df_pivot['FINAL'] - df_pivot['QTYTEORI']

            st.dataframe(
                df_pivot[['BARCODE_KODE', 'DESKRIPSI', 'TITIKLOKASI', 'QTYTEORI', 'P1', 'P2', 'P3', 'FINAL', 'SELISIH']],
                use_container_width=True,
                column_config={
                    "P1": st.column_config.NumberColumn(format="%d", help="Hasil Hitung P1"),
                    "P2": st.column_config.NumberColumn(format="%d", help="Hasil Hitung P2"),
                    "P3": st.column_config.NumberColumn(format="%d", help="Hasil Hitung P3"),
                    "QTYTEORI": "TEORI",
                    "FINAL": st.column_config.NumberColumn("FINAL", format="%d", background_color="#e1f5fe")
                }
            )
